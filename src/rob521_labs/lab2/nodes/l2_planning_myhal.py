...
class PathPlanner:
    ...
    def __init__(self, map_filename, map_setings_filename, goal_point, stopping_dist):
        ...
        # print('self.occupancy_map.shape', self.occupancy_map.shape)
        #Pygame window for visualization
        self.window = pygame_utils.PygameWindow(
            "Path Planner", (1590, 490), self.occupancy_map.shape, self.map_settings_dict, self.goal_point, self.stopping_dist, map='myhal') #(1590, 490)
        return
    ...
    def sample_map_space_bri(self): # bridge
        ...
        self.window.add_point(np.array(sample).reshape(2,), radius=2, width=0, color=(0, 0, 255)) # blue
        return sample
    ...
    def sample_map_space_simple(self):
        ...
        self.window.add_point(np.array(sample).reshape(2,), radius=2, width=0, color=COLORS['b'])
    ...
    def sample_map_space(self, mode='rrt'):
        ...
        # print("TO DO: Sample point to drive towards")
        goal_bias_rate = 0.5
        sample = self.goal_point #np.zeros((2, 1))
        # tight_bounds = np.array([[-0.2, 7.75], 
        #                          [-0.2, 2.25]])
        # # self.window.add_point(np.array([5, -5]).reshape(2,), radius=7, width=0, color=COLORS['r'])
        # if np.random.rand() < goal_bias_rate:
        #     # Use goal-sampling occasionally
        #     # sample = self.goal_point
        ...
        #     self.window.add_point(np.array(sample).reshape(2,), radius=2, width=0, color=COLORS['r'])
        #     return sample
        # else:
        #     # Use uniform random sampling
        #     # Selecting a tighter bound from the most recent point to the goal point 
    ...
    def rrt_planning(self):
        ...
        for i in range(8000): #Most likely need more iterations than this to complete the map!
...
def main():
    ...
    #robot information
    goal_point = np.array([[7], [2]]) #m # np.array([[10], [10]])
    ...
    np.save("myhal_rrt_star.npy", node_path_metric)
    ...
    pygame.image.save(path_planner.window.screen, "myhal_rrt_star.png")
